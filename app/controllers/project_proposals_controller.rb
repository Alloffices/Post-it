class ProjectProposalsController < ApplicationController
  ...
  def new
    @project_proposal = current_user.project_proposals.build
  end
  ...
  def create
    @project_proposal = current_user.project_proposals.build(project_proposal_params)
  ...
  private
    ...
    def project_proposal_params
      params.require(:project_proposal).permit(:title, :description)
    end
end
